...
def LSM_american(prices, option, discount_rate = 0.06):
        ...
        for path in range(n):
            ...
            if payoff > 0:
                ...
                if cf == 0:
                    ...
                    #Y.append(cash_flow[path,cf]*((0.94176)**(cf-t)))
    ...
    #present_values = np.max(cash_flow[:])/((1+discount_rate)**(np.argmax(cash_flow[:])))
    for path in range(n):
        cf = np.argmax(cash_flow[path])
        #present_values[path] = cash_flow[path,cf]*((0.94176)**(cf-t))
